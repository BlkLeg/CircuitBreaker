...
```text
python/                         # python-build-standalone interpreter + site-packages
bin/circuit-breaker             # launcher: exec python -m app.start
bin/cb-python                   # interpreter wrapper
share/
  VERSION
  build-info.json
  DocsPage.md
...
`scripts/build_native_release.py` assembles a python-build-standalone tree; the glibc floor is
measured from the tree and recorded in `share/build-info.json`. Build on the oldest distribution
...
**Recommended / first-class:** `install.sh` native (and Proxmox, which uses it inside an LXC).
**Advanced / manual:** distro packages (deb/rpm/apk). Both write
`install-identity.json` and install the same `/usr/local/bin/cb`. See
[`specs/install/compatibility-matrix.yaml`](../specs/install/compatibility-matrix.yaml).

### Distro package (.deb / .rpm / .apk) layout (advanced)

Paths shipped by `nfpm.yaml` and created by `packaging/postinstall.sh`:

- Application tree: `/opt/circuitbreaker/{python,bin,share,agent-binaries}`
- Binary (symlink): `/usr/local/bin/circuit-breaker` → `/opt/circuitbreaker/bin/circuit-breaker`
- CLI: `/usr/local/bin/cb`
- Identity: `/etc/circuit-breaker/install-identity.json`
- Historical share paths: `/usr/local/share/circuit-breaker/*` (compat symlinks into the tree)
- Config: `/etc/circuit-breaker/config.toml`
- Env file: `/etc/circuit-breaker/circuit-breaker.env` (generated by `postinstall.sh`; read by the unit's `EnvironmentFile=-`)
...
The generated config file is TOML and drives host/port, data paths, worker count, and optional TLS cert/key paths. The env file carries the values `postinstall.sh` generates: `CB_DB_URL`, `CB_VAULT_KEY`, `CB_REDIS_URL`, `NATS_AUTH_TOKEN`, `STATIC_DIR`, `CB_SHARE_DIR`, `CB_ALEMBIC_INI`, and `CB_AGENT_BINARIES_DIR`.

### `install.sh` native layout

The `curl | bash` installer uses the same application tree at `/opt/circuitbreaker`:
