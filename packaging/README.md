...
The generated config file is TOML and drives host/port, data paths, worker count, and optional TLS cert/key paths. The env file carries the values `postinstall.sh` generates: `CB_DB_URL`, `CB_VAULT_KEY`, `CB_REDIS_URL`, `NATS_AUTH_TOKEN`, `STATIC_DIR`, `CB_ALEMBIC_INI`, and `CB_AGENT_BINARIES_DIR`.
