...
# When invoked as `curl | bash`, stdin is the pipe — redirect to terminal
[[ -t 0 ]] || exec < /dev/tty

# ── Logging ───────────────────────────────────────────────────────────────────

# Strip ANSI codes from log file, tee to terminal (guard if /dev/tty unavailable)
if [[ -e /dev/tty ]]; then
...
phase5_configure_integration() {
  ...
  # Build JSON safely via python3 to handle special chars in token secret
...
print(json.dumps({
  ...
  [[ -n "$json_payload" ]] || cb_fail "Failed to build integration JSON payload" \
    "Ensure python3 is available on the PVE host"
