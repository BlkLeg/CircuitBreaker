...
//
// A successful public-mode retry passes publicRetryIndex and lands in the
...
- [ ] **Step 7: Implement the public-mode cutover retry**
...
func TestPublicSuccessorRetry_IsAllowedOnlyWhenAdvertised(t *testing.T) {
	pinned := tlsdial.Trust{Mode: tlsdial.ModeSelfSigned, Pins: []string{"PIN-A"}}
	advertised := tlsdial.Trust{
		Mode: tlsdial.ModeSelfSigned, Pins: []string{"PIN-A"}, PublicSuccessorPending: true,
	}

	if shouldRetryPublic(pinned) {
		t.Error("shouldRetryPublic without an advertised public successor = true, want false")
	}
	if !shouldRetryPublic(advertised) {
		t.Error("shouldRetryPublic with an advertised public successor = false, want true")
	}
}

// A public-mode trust has nothing to retry — it already uses standard
// verification on the first attempt.
func TestPublicSuccessorRetry_NotOfferedInPublicMode(t *testing.T) {
	if shouldRetryPublic(tlsdial.Trust{Mode: tlsdial.ModePublic}) {
		t.Error("shouldRetryPublic in public mode = true, want false")
	}
}
...
func TestPromoteTrust_PublicRetryReportsSuccessorAndClears(t *testing.T) {
	...
	kind, err := PromoteTrust(&config.Config{TLSPin: "PIN-A"}, dir, publicRetryIndex)
	if err != nil {
		t.Fatalf("PromoteTrust: %v", err)
	}
	if kind != "successor" {
...
Run: `cd apps/agent && go test -race ./internal/link/... -run 'PublicSuccessor|PublicRetry'`
Expected: FAIL — `undefined: shouldRetryPublic`, `undefined: publicRetryIndex`.

Then implement in `apps/agent/internal/link/trust.go`:

```go
// publicRetryIndex is the candidate index PromoteTrust is given after a
// successful public-mode retry. Any index above zero means "not the current
// policy", and there is no pin candidate to point at in public mode, so this
// is a named constant rather than a magic 1.
const publicRetryIndex = 1

// shouldRetryPublic reports whether a failed pinned dial should be retried
// once with standard system-CA verification.
//
// True only when the server advertised a move to public trust over the
// already-authenticated link and the agent persisted it. That precondition is
// what keeps this from being a trust-on-first-use fallback: an attacker who
// can merely make pinned dials fail gets nothing, because they cannot make the
// agent believe a public cutover was announced.
func shouldRetryPublic(trust tlsdial.Trust) bool {
	return trust.Mode == tlsdial.ModeSelfSigned && trust.PublicSuccessorPending
}
```

In `link.go`'s dial path, when the pinned dial fails and `shouldRetryPublic(trust)`
is true, retry once with `tlsdial.NewDialer(tlsdial.Trust{Mode: tlsdial.ModePublic})`.
On success, call `PromoteTrust(opts.Config, opts.StateDir, publicRetryIndex)` and
use its returned kind as `tlsPinKind`. On failure, return the **original** pinned
error — it is the one that describes what the operator actually has to fix.
