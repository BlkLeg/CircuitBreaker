...
Run: `cd apps/agent && go test -race ./internal/tlsdial/...`
Expected: PASS (7 tests). `go build ./...` still fails for `internal/link`, `internal/enroll` and `internal/update` — Task 4 fixes those.
...
- [ ] **Step 7: Run the full agent suite and the Python conformance suite**
...
In `apps/backend/src/app/api/agents.py`, after `get_binary`:
...
def test_no_private_key_material_is_committed() -> None:
    ...
    and fixtures. Every test generates its keypair at runtime."""
    offenders: list[str] = []
    markers = ("BEGIN PRIVATE KEY", "BEGIN OPENSSH PRIVATE KEY", "BEGIN EC PRIVATE KEY")
    for path in REPO_ROOT.rglob("*"):
        if not path.is_file() or ".git/" in path.as_posix() or "node_modules" in path.as_posix():
            continue
        try:
...
If `test_no_private_key_material_is_committed` fails, that is a genuine finding: investigate before proceeding rather than adjusting the test.
