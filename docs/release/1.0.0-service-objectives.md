...
| Degraded | Core inventory is available, but optional capability is unavailable or stale. | Read inventory, edit inventory when DB is healthy, view known degradation. | Dispatching work through the degraded capability unless the operation has an explicit fallback. | **Not implemented.** `ServerState` is `STARTING \| READY \| STOPPING`; there is no degraded state and no endpoint reports one. Per-dependency detail is visible in `readyz`'s `checks` map, but a partial outage is reported as not-ready, not as degraded. Adding the state requires an ADR and is tracked under SRV-03. |
| Not ready | Required dependency, schema, vault, or migration state prevents safe writes. | Health/diagnostics and documented recovery commands. | Writes, destructive admin actions, background dispatch, agent work. | **Not implemented.** SRV-03 requires that "readiness rejects writes when they cannot be served safely." Nothing rejects them: `get_state()` is read only inside three of the four health handlers (`startupz`, `readyz` and the legacy `health` — `livez` reads none), and no middleware or dependency refuses a write while `readyz` is 503. An operator or load balancer must act on the signal; the server will not. Tracked under SRV-03. |
...
operations are safe. Two rows above do not yet meet that bar: **Degraded** has no representation in
the server at all, and **Not ready** is advertised but not enforced. Until both are closed, this
table describes the contract RC-05 defines, not a contract the running server guarantees.
...
RC-05's acceptance also requires that "SRV-03 and REL-21 tests produce the named metrics." **No named
metric in the tables below is emitted today.** `/api/v1/metrics` exposes inventory counts and
per-service status only; there is no HTTP request, latency, readiness-transition or queue-depth
instrumentation, and no REL-21 test exists to produce one. Every objective below is therefore a
published target without a measurement source in the product, which is why RC-05 remains
`in_progress` and unevidenced in the requirement ledger.
