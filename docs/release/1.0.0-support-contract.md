...
| npm distribution                                                 | Deferred until NPM slices pass | No npm package is part of the supported 1.0.0 surface until NPM-1 through NPM-6 pass            | NPM owner      |
...
| Docker Compose install             | Supported candidate   | Linux `amd64` and `arm64` with Docker Engine and Compose plugin v2                                      | ACC-2, ACC-3, ACC-6 |
...
| Database                 | Supported candidate                | SQLite for single-node lab installs; PostgreSQL for production candidate installs                                                   | ACC-2, ACC-5, ACC-6 |
...
| Multi-tenancy            | Unsupported as a security boundary | Tenant UI/data fields must not be represented as isolation in 1.0                                                                   | SEC-1, SEC-2A/2B    |
