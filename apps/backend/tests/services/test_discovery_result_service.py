...
# ── Normalization, now symmetric across both paths ────────────────────────────


def test_server_path_matches_a_lowercase_mac_against_an_uppercase_stored_one(  # type: ignore[no-untyped-def]
    db_session, factories
) -> None:
    """Normalization used to be agent-only, on the stated grounds that doing it
    on the server path would change that path's SQL and drop its index. Neither
    was true — it changes a bound parameter, and there was no index until
    migration 0109 added one — and the asymmetry meant an ARP or OPNsense row,
    both of which report lowercase, could not match a device the nmap scanner
    had written in uppercase."""
    hw = factories.hardware(name="lower-mac", mac_address="AA:BB:CC:10:00:20")
    job = _job(db_session, tenant_id=None)

    res, classification = build_and_classify_result(
        db_session, job, {"ip": "10.60.0.20", "mac_address": "aa:bb:cc:10:00:20"}
    )
    db_session.flush()

    assert classification == "matched"
    assert res.matched_entity_id == hw.id
    assert res.mac_address == "AA:BB:CC:10:00:20"


def test_server_path_normalizes_ipv6_before_matching(db_session, factories) -> None:  # type: ignore[no-untyped-def]
    hw = factories.hardware(name="v6", ip_address="2001:db8::1")
    job = _job(db_session, tenant_id=None)

    res, classification = build_and_classify_result(
        db_session, job, {"ip": "2001:0DB8:0000:0000:0000:0000:0000:0001"}
    )
    db_session.flush()

    assert classification == "matched"
    assert res.matched_entity_id == hw.id
    assert res.ip_address == "2001:db8::1"


def test_a_dotted_mac_is_repunctuated_into_the_form_hardware_stores(db_session, factories) -> None:  # type: ignore[no-untyped-def]
    """`normalize_mac` delegates to `_norm_mac`, which is the function that wrote
    every `hardware.mac_address` an accept has produced — so the lookup key and
    the stored value are generated by the same code. Twelve hex digits uniquely
    determine an address, so this cannot reshape one host into another."""
    hw = factories.hardware(name="dotted", mac_address="AA:BB:CC:10:00:21")
    job = _job(db_session, tenant_id=None)

    res, classification = build_and_classify_result(
        db_session, job, {"ip": "10.60.0.21", "mac_address": "aabb.cc10.0021"}
    )
    db_session.flush()
...
def test_an_unrecognisable_mac_is_left_unmatched_rather_than_reshaped(db_session) -> None:  # type: ignore[no-untyped-def]
    job = _job(db_session, tenant_id=None)

    res, classification = build_and_classify_result(
        db_session, job, {"ip": "10.60.0.22", "mac_address": "not-a-mac"}
    )
    db_session.flush()

    assert classification == "new"
    assert res.mac_address == "NOT-A-MAC"


def test_the_classifier_still_only_classifies(db_session, factories) -> None:  # type: ignore[no-untyped-def]
    """Enrichment is the caller's job, not the classifier's: `_scan_import` and
    `agent_discovery` each call `discovery_enrich` after this returns, so this
    function stays a pure verdict and the row it builds is still `pending`."""
    hw = factories.hardware(name="known", mac_address="AA:BB:CC:10:00:23", vendor=None)
    job = _job(db_session, tenant_id=None)

    res, classification = build_and_classify_result(
        db_session, job, {"ip": "10.60.0.23", "mac_address": "AA:BB:CC:10:00:23"}
    )
    db_session.flush()

    assert classification == "matched"
    assert res.merge_status == "pending"
