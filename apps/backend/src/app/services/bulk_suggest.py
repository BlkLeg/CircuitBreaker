...
duplicate detection, and service naming suggestions for multi-select
merge operations.
"""
...
def suggest_bulk_actions(db: Session, result_ids: list[int]) -> dict:
    ...
    """
    results = db.execute(select(ScanResult).where(ScanResult.id.in_(result_ids))).scalars().all()
...
def _suggest_networks(db: Session, results: Sequence[ScanResult]) -> list[dict]:
    ...
    for cidr, members in subnet_groups.items():
        ...
        existing = db.execute(select(Network).where(Network.cidr == cidr)).scalar_one_or_none()
...
def _suggest_catalog_matches(results: Sequence[ScanResult]) -> dict[int, dict]:
...
def _detect_duplicates(db: Session, results: Sequence[ScanResult]) -> list[dict]:
    ...
    for r in results:
        ...
        existing = db.execute(select(Hardware).where(or_(*conditions))).scalars().first()
...
def _suggest_services(results: Sequence[ScanResult]) -> dict[int, list[dict]]:
