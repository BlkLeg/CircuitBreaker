"""Zero-configuration discovery bootstrap.
...
that agent's own segments starts: the five steps — normalize the reported
...
**Every write goes through `discovery_profiles_service`**. That module
owns the closed field list, derives `normalized_cidr`, is wherethe
...
of them here. The one judgement this module adds is the grant's
...
**Ordering.** The trigger is `agent_registry.record_network_facts`, which
...
# It is also the predicate ofthe partial unique index
...
# are that one, and this is the single post-approval scan the contract step 4 asks
# for, which is the one an operator is watching for after enrolment.
BOOTSTRAP_TRIGGERED_BY = "bootstrap"

# An agent profile exposes the focused connect scan and nothing else.
...
# Step 5: "schedule recurring scans every six hours with per-agent
...
# encoded in the expression itself — hence the derived minute. Derived
# once, for a brand-new profile only: re-deriving on every pass would revert
# the "edit cadence and scan depth" the moment a readiness frame arrived,
# which is at most fifteen minutes later.
RECURRING_SCAN_INTERVAL_HOURS = 6
_CRON_MINUTE_MODULUS = 60

# Step 4's "short jitter". A floor, because approval and the first
...
# small: the operator is watching, and the contract wants approval to "produce useful
# data promptly".
...
def system_profile_cron(agent_id: int) -> str:
    """The recurring cadence a brand-new system profile is created with."""
...
def initial_scan_delay_s(agent_id: int) -> float:
    ...
    Derived from the agent id rather than drawn at random, for the reason the contract
...
def eligible_subnets(scope: agent_scope.EffectiveScope, *, address_ceiling: int) -> list[str]:
    """The directly connected subnets that may become automatic scope.
    ...
    deciding something the operator did not ask for. the contract step 6 is about a
...
async def run_bootstrap(db: Session, agent_id: int) -> BootstrapOutcome:
    ...
    the point — the contract requires that "repeated hello/readiness frames must not
    ...
    `require_online=False`, matching every other creation-time checkpoint:
    ...
    for row in stored:
        # A system row with no resolvable subnet cannot be keyed on, andthe
    ...
    # the closing paragraph, read on step 4: queueing a scan *is*
    ...
    # Appearances first, disappearances second — the contract step 6's own order, so
    ...
    for cidr in wanted:
        ...
        try:
            ...
            # that wonthe unique index — must not cost this agent its other
...
def _system_profiles(db: Session, agent_id: int) -> list[DiscoveryProfile]:
    ...
    The `managed_by` predicate is the whole of the "user-created profiles
    ...
    neither re-tuned nor disabled by an automatic pass. the partial unique
...
def _create_system_profile(db: Session, agent: Agent, cidr: str) -> DiscoveryProfile:
    """Mint the automatic profile for one subnet (the contract step 3).
    ...
    which is what makes the upsert idempotent againstthe unique index rather
...
def _queue_initial_scan(db: Session, agent: Agent, profile: DiscoveryProfile, cidr: str) -> int:
    """One bounded scan, shortly after the subnet appears.
    ...
    execution-location checkpoint and the same tenant derivation that a
    ...
    re-enabled). A pass over an unchanged report queues nothing, because
...
def schedule_bootstrap(agent_id: int) -> bool:
    ...
    this pass commits (the contract, and the module docstring): a profile created and a
