"""One agent dispatch, end to end: `discovery.request` out, `discovery.finding` in.
...
agent parks the job instead of failing it, and the request is published
...
3. **The lease, against server receipt time.** A spooled frame keeps its original
   ...
   reconciler has already given the dispatch up and derives its own
   grace from this module's constant.
4. **Targets, then the scope snapshotted on the job.** The agent's *live*
...
**Log hygiene.** `hostname`, `banner` and `evidence` are untrusted
...
and touching none of the counters the host findings accumulated.
...
# owns it because every the contract checkpoint already calls that module, and a second
...
# the limit for one finding, enforced on the raw mapping before validation.
...
# derives its own grace from this constant rather than restating it,
...
# The `scan_jobs.error_reason` vocabulary the agent execution path writes.
...
# and nowhere else, so they are named separately from the the contract vocabulary above.
...
# The agent path increments, because it has no finished batch to write
...
# writes. `running` is the agent's own progress signal and `expired` is the;
...
# How long a job may sit waiting for an agent that is not connected before
# the reconciliation pass gives it up as `agent_unavailable`. Fifteen
...
# `scan_jobs.progress_phase` while a claim is parked. The job stays
...
# The closed `discovery.request` method vocabulary, in the agent's own
...
# ever". Everything else it can answer is a configuration fault that fails
...
# Every other denial, mapped onto the the contract vocabulary. An eligibility reason the
...
#: `status` is a bare string the history filter, the history query and the
...
# prose, for the reason the whole the contract vocabulary is: without it the row reads as
...
class InvalidDiscoveryFinding(ValueError):
    ...
    The `agent_link` handler catches this, rate-limits it through
...
def _reason(code: str, address: str | None = None) -> str:
    ...
    the payload the rule exists for, and the answer is to keep the untrusted
...
async def dispatch_discovery_job(db: Session, job_id: int) -> bool:
    """Turn one queued agent job into one delivered `discovery.request`.
    ...
    * **An offline agent parks the job instead of failing it**. The claim
      is given back — `queued`, `waiting_for_agent`, and a deadline the
    ...
    * **Publication happens after the commit**, unlike `probe.assign`. An agent
    ...
    # caps `targets` at `MAX_DISCOVERY_TARGETS` and
    ...
    # Refusing here closes it with a the contract reason instead. Creation time asks the
    ...
    # re-asked against the *live* grant through the same readersthe
    ...
    admitted = cast(
        ...
        db.execute(
            ...
            .values(
                ...
                # The version in force when the request was built. Ingest
...
def _release_to_waiting(db: Session, job: ScanJob, reason: str, *, dispatch_id: str) -> bool:
    """Give the claim back and let the job wait for its agent.
...
#   nobody closed here stays open until the pass expires it.
...
class JobCancelOutcome(NamedTuple):
    ...
    ambiguity: a job parked in `waiting_for_agent` holds no lease and so
...
# The job status a server-initiated cancellation writes. Not `failed`: the
...
def _validated_reason(reason: str | None) -> str | None:
    """the vocabulary is closed. A reason outside it is a programming error
...
def _open_agent_jobs(
    ...
    `waiting_for_agent` holds no lease, but its agent will never run it
    once the grant is off or the profile is disabled, and leaving it queued
    would have the pass dispatch it later.
    """
...
def _transition_terminal(
    ...
    *completed* into one that was cancelled and emits a `discovery.cancel` for
    ...
    # asked for is not one ofthe error outcomes, and the status alone says so.
...
def cancel_agent_dispatches(
    ...
    """Retire every dispatch this agent holds. Caller owns the transaction.

    The trigger for `local_discovery` being turned off and for revocation, and
    the one the design and `agent_link` reach for when an agent goes away for good.
    """
...
def cancel_profile_dispatches(
    ...
    """Retire whatever this profile has in flight. Caller owns the transaction.

    Two callers, deliberately one implementation:
    `discovery_profiles_service.update_profile` when an administrator disables a
    profile, and the subnet-disappearance path — which disables the system
    profile for a subnet the agent stopped reporting. the contract names profile-disable
...
def cancel_scope_changed_dispatches(db: Session, agent_id: int) -> DiscoveryCancellation:
    """Retire the dispatches this agent's *current* scope no longer authorizes.
...
def _scope_moved_under(job: ScanJob, scope: agent_scope.EffectiveScope) -> bool:
    ...
    refuses every finding arriving under any other one, so a dispatch
    whose version has moved can only starve. Its targets are not re-checked,
    because the version already covers them.

    A job with no lease — parked in `waiting_for_agent`, or queued and
...
def validate_finding_payload(payload: dict[str, Any]) -> DiscoveryFindingPayload:
    ...
    try:
        ...
        # text the contract forbids in an audit detail.
...
async def ingest_discovery_finding(
    ...
    *,
    ...
    # down. Reading them twice — `_authorized_address` deriving the scope (a
    # grant read plus an `agent_networks` read) and `_record_host_finding`
    # reading the grant again for the ceiling — is three round trips per finding
    # on the path the module docstring promises is "a handful of indexed
    # lookups". Resolving here keeps the security properties identical: the
...
async def _finalize_from_summary(
    ...
    and are deliberately *not* written to the job: the contract makes the agent path's
...
def _summary_error_text(
    ...
    reviewable. the contract gives that no status of its own, so this is the only place
...
def _authenticated_job(db: Session, agent: Agent, finding: DiscoveryFindingPayload) -> ScanJob:
    ...
    if job.tenant_id != agent.tenant_id:
        # Tenant context is derived from the job/agent, never accepted
...
def _authorized_address(
    ...
    *scope* is derived by the caller once per frame and is the agent's **live**
    scope, exactly as before — the the contract rule below is what makes that safe, and
    ...
    if scope.version != job.scope_version:
        # The live scope is not the authority: a sender that can move its
        ...
        # sender controls; the design cancels the dispatch on the same signal.
...
async def _close_over_ceiling(db: Session, agent: Agent, job: ScanJob, ceiling: int) -> None:
    ...
    the `agent_execution_error`: an agent producing more findings than its own
    address budget allows is not executing the job it was given. the
