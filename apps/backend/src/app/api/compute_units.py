...
async def upload_icon(
    ...
    if not _verify_magic_bytes(data, file.content_type):
        raise HTTPException(
            status_code=415, detail="File content does not match the declared content type."
        )
...
def delete_icon(slug: str, db: Session = Depends(get_db), _=Depends(require_write_auth)):
    ...
    db_icon = (
        db.query(UserIcon).filter((UserIcon.slug == slug) | (UserIcon.filename == slug)).first()
    )
    if db_icon:
        db.delete(db_icon)
        db.commit()
...
def delete_compute_unit(
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Cannot delete: other records still reference this compute unit.",
        ) from exc
