...
def delete_service(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=409, detail="Cannot delete: record is still referenced by other entities."
        ) from exc
...
def get_external_deps(service_id: int, db: Session = Depends(get_db)):
    ...
    links = (
        db.execute(select(ServiceExternalNode).where(ServiceExternalNode.service_id == service_id))
        .scalars()
        .all()
    )
    result = []
...
@router.post(
    "/{service_id}/external-dependencies", response_model=ServiceExternalNodeRead, status_code=201
)
