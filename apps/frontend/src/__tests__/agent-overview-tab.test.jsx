...
describe('AgentOverviewTab', () => {
  ...
  // Slice A: the address the agent dialed. Without it, an agent that came in
  // through a stale endpoint looks exactly like one that never had a problem.
  it('names the address the agent enrolled through', () => {
    renderOverview({
      panels: ['enrollment'],
      agent: { ...AGENT, enrolled_via_endpoint: 'https://cb.example.com' },
    });
    expect(screen.getByRole('region', { name: 'Enrollment' })).toBeTruthy();
    expect(screen.getByText('https://cb.example.com')).toBeTruthy();
  });

  it('dashes the address for an agent that enrolled before it was recorded', () => {
    renderOverview({ panels: ['enrollment'], agent: { ...AGENT } });
    const term = screen.getByText('Enrolled via');
    expect(term.nextSibling.textContent).toBe('—');
  });
