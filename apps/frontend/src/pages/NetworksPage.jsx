    ...
    <div className="page">
      ...
      {hwFilter && (
        <div className="info-tip" style={{ marginBottom: 8 }}>
          🔍 Showing all networks — open a network row to see which members run on{' '}
          <strong>{hardware.find((h) => String(h.id) === hwFilter)?.name}</strong>.
        </div>
      )}

      {!loading && items.length === 0 && settings?.show_page_hints && (
        <div className="info-tip" style={{ marginBottom: 8 }}>
          💡 <strong>Tip:</strong> It&apos;s recommended to add hardware first, then create
          networks. You can assign a hardware node as the gateway for each network.
        </div>
      )}
