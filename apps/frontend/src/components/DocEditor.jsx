  ...
  const headingCommand = {
    ...
    icon: (
      <select
        ...
        onChange={(e) => {
          headingLevelRef.current = e.target.value;
          e.target.value = '';
        }}
        defaultValue=""
      >
    ...
    execute: (state, api) => {
      ...
      if (!level || level === 'p') {
        api.replaceSelection(state.selectedText.replace(/^#{1,6} /, ''));
        return;
      }
    ...
    <div className="doc-editor-wrapper" data-color-mode="dark">
      ...
      {draftBanner && (
        <div className="doc-editor-draft-banner">
          ...
          <button className="restore" onClick={restoreDraft}>
            Restore draft
          </button>
      ...
      <div className="doc-editor-status">
        ...
        <span className="doc-editor-wordcount">
          {wordCount > 0 &&
            `${wordCount.toLocaleString()} word${wordCount === 1 ? '' : 's'} · ~${readingMins} min read`}
        </span>
