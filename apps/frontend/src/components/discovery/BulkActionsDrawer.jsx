...
export default function BulkActionsDrawer({ results, onClose, onComplete }) {
  ...
  useEffect(() => {
    ...
    suggestBulkActions({ result_ids: ids })
      ...
      .catch(() => {
        if (!abortController.signal.aborted) toastRef.current.error('Failed to load suggestions');
      })
      .finally(() => {
        if (!abortController.signal.aborted) setLoadingSuggestions(false);
      });
  ...
  const filteredVendors = useMemo(() => {
    ...
    return entries
      .filter(
        ([key, v]) => key.toLowerCase().includes(q) || (v.name || '').toLowerCase().includes(q)
      )
      .slice(0, 20);
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    const bulkActionId = `bulk_${results.length}_${Date.now()}`;
    const resultCount = results.length;

    try {
      // Optimistic update: immediately decrement badge by number of results
      discoveryEmitter.emit('badge:decrement', { count: resultCount, actionId: bulkActionId });
      ...
      // Adjust badge count based on actual results if different from optimistic
      const actualProcessed = accepted;
      if (actualProcessed !== resultCount) {
        const adjustment = resultCount - actualProcessed;
        if (adjustment > 0) {
          discoveryEmitter.emit('badge:increment', { count: adjustment });
        }
      }

      toast.success(
        `${accepted} host${accepted !== 1 ? 's' : ''} merged${skipped ? `, ${skipped} skipped` : ''}`
      );
      onComplete(res.data);
    } catch (err) {
      // Revert optimistic update on error
      discoveryEmitter.emit('badge:increment', { count: resultCount });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>Bulk Actions</h2>
          ...
          {loadingSuggestions ? (
            ...
            <>
              ...
              <Section
                ...
                badge={suggestions?.clusters?.length || null}
                defaultOpen={!!suggestions?.clusters?.length}
              >
                <label
                  style={{
                    display: 'flex',
                  ...
                  {['none', 'new', 'existing'].map((m) => (
                    ...
                    >
                      <input
                        ...
                        name="networkMode"
                        value={m}
                        checked={networkMode === m}
                        onChange={() => setNetworkMode(m)}
                      />
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <input
                        ...
                        placeholder="VLAN"
                        value={networkVlan}
                        onChange={(e) => setNetworkVlan(e.target.value)}
                        style={{ width: 80 }}
                      />
                    </div>
                    <input
                      className="form-control"
                      placeholder="Gateway (e.g. 10.0.0.1)"
              ...
              <Section
                ...
                badge={Object.keys(suggestions?.catalog_matches || {}).length || null}
              >
                ...
                {suggestions?.catalog_matches &&
                  Object.keys(suggestions.catalog_matches).length > 0 && (
                    <div style={{ marginTop: 8, fontSize: 11, color: 'var(--color-text-muted)' }}>
                      <Check
                        size={11}
                        style={{ display: 'inline', marginRight: 4, color: '#22c55e' }}
                      />
                      {Object.keys(suggestions.catalog_matches).length} host(s) auto-matched to
                      vendor catalog
                    </div>
                  )}
              </Section>
              ...
              <Section
                ...
                badge={Object.keys(roleSummary).length || null}
              >
              ...
              <Section icon={Zap} title="Detected Services" badge={servicesByPort.length || null}>
                ...
                >
                  <input
                    ...
                    onChange={(e) => setCreateServices(e.target.checked)}
                  />
                ...
                {servicesByPort.length > 0 ? (
                  ...
                  <p style={{ fontSize: 12, color: 'var(--color-text-muted)', margin: 0 }}>
                    No services detected on open ports.
                  </p>
                )}
              </Section>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            >
              {submitting
                ? 'Merging…'
                : `Accept & Merge ${results.length} Host${results.length !== 1 ? 's' : ''} →`}
            </button>
          </div>
        </div>
      </motion.div>
