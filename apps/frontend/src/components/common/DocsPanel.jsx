          ...
          {unattachedDocs.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {unattachedDocs.map((doc) => (
                ...
                >
                  <span>{doc.title}</span>
                  <button className="btn btn-xs btn-primary" onClick={() => handleAttach(doc.id)}>
                    Link
                  </button>
                </div>
              ))}
      ...
      {loading ? (
        ...
        >
          ...
          {attachedDocs.length === 0 && !loading && (
            <p
              style={{ fontStyle: 'italic', color: 'var(--color-text-muted)', fontSize: '0.9rem' }}
            >
        ...
        <div className="modal-overlay" onClick={() => setViewDoc(null)}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3>{viewDoc.title}</h3>
              <button className="btn btn-sm" onClick={() => setViewDoc(null)}>
                Close
              </button>
            </div>
            <div
