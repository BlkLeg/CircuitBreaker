...
export function getEdgeColor(relation) {
  ...
  return EDGE_COLORS[relation] || '#6c7086';
}
...
export const FILTER_NODE_TYPES = Object.keys(NODE_STYLES).filter(
...
export function resolveNodeIcon(type, icon_slug, vendor, kind, role, clusterType) {
  ...
  if (type === 'hardware' && ROLE_ICON[role]) {
    return getIconEntry(ROLE_ICON[role])?.path ?? null;
  }
  ...
  if (kind && KIND_ICON[kind]) {
    return getIconEntry(KIND_ICON[kind])?.path ?? null;
  }
...
export const ENTITY_API_UPDATE_ICON = {
...
export const ENTITY_API_UPDATE_STATUS = {
...
export const ENTITY_API_UPDATE_ALIAS = {
...
export const ENTITY_API_DELETE = {
...
export const STATUS_OPTION_LABEL = {
  auto: '🧭 Auto (derived)',
  online: '🟢 Online',
  offline: '🔴 Offline',
  degraded: '🟠 Degraded',
...
export const LEGACY_LABEL_COLOR_MAP = {
  yellow: 'yellow',
  blue: 'blue',
  green: 'green',
  pink: 'red',
