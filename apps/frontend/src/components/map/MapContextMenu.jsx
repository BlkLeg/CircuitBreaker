...
const ICON_SUPPORTED_TYPES = new Set([
  'cluster',
  'hardware',
  'compute',
  'service',
...
function getSublabel(entity, type) {
  ...
  if (type === 'storage')
    return entity.kind || (entity.capacity_gb ? `${entity.capacity_gb} GB` : null);
  if (type === 'network') return entity.cidr || null;
  if (type === 'misc') return entity.kind || null;
  if (type === 'cluster')
    return (
      entity.environment || (entity.member_count > -1 ? `${entity.member_count} members` : null)
    );
  ...
  return (
    <>
      ...
      >
          ...
          <div key={targetType}>
            <button
              ...
              style={{
                width: '100%',
                background:
                  activeSubmenu === targetType ? 'var(--color-secondary)' : 'var(--color-surface)',
                border: 'none',
                color: 'var(--color-text)',
                padding: '9px 14px',
                display: 'flex',
            ...
            {activeSubmenu === targetType && (
              ...
              >
                ...
                <div style={{ maxHeight: 180, overflowY: 'auto' }}>
                  {optionsError && (
                    <div
                      style={{ padding: '8px 12px', fontSize: 11, color: 'var(--color-danger)' }}
                    >
                      {optionsError}
                    </div>
                  )}
                  ...
                  {filteredOptions.map((entity) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {sublabel && (
                          <span style={{ color: 'var(--color-text)', fontSize: 10 }}>
                            {sublabel}
                          </span>
                        )}
                      </button>
          ...
          <>
            ...
            {linkedItems.map(({ edge, label }) => (
              <button
                ...
                onMouseEnter={(e) => {
                  if (!unlinkingEdgeId) e.currentTarget.style.background = 'var(--color-secondary)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = 'var(--color-surface)';
                }}
        ...
        {(linkItems.length > 0 || linkedItems.length > 0) &&
            ...
            return (
              <>
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!statusOpen) e.currentTarget.style.background = 'var(--color-secondary)';
                  }}
                  onMouseLeave={(e) => {
                    if (!statusOpen) e.currentTarget.style.background = 'var(--color-surface)';
                  }}
                >
