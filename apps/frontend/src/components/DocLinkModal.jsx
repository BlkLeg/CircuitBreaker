...
const ENTITY_TYPES = [
  {
    ...
    label: 'Hardware',
    icon: '🖥',
    api: (q) => hardwareApi.list(q ? { q } : undefined),
  },
  {
    key: 'service',
    label: 'Services',
    icon: '⚙️',
    api: (q) => servicesApi.list(q ? { q } : undefined),
  },
  {
    key: 'compute',
    label: 'Compute',
    icon: '💻',
    api: (q) => computeUnitsApi.list(q ? { q } : undefined),
  },
  {
    key: 'storage',
    label: 'Storage',
    icon: '💾',
    api: (q) => storageApi.list(q ? { q } : undefined),
  },
  {
    key: 'network',
    label: 'Networks',
    icon: '🌐',
    api: (q) => networksApi.list(q ? { q } : undefined),
  },
  {
    key: 'environment',
    label: 'Environments',
    icon: '🏷',
    api: (q) => environmentsApi.list(q ? { q } : undefined),
  },
];
...
function DocLinkModalContent({
  ...
  const linkedIds = useMemo(
    () =>
      new Set(existingLinks.filter((l) => l.entity_type === activeType).map((l) => l.entity_id)),
