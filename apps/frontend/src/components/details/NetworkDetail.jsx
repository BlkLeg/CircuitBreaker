...
function NetworkDetail({
  ...
  onClose,
  hardwareFilter = null,
  hardware: hwProp = null,
  computeUnits: cuProp = null,
}) {
  ...
  useEffect(() => {
    if (hwProp) setHardware(hwProp);
  }, [hwProp]);
  useEffect(() => {
    if (cuProp) setComputeUnits(cuProp);
  }, [cuProp]);
  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title={`Network: ${network.name}`}>
      <div className="tabs">
        ...
        >
          Members{' '}
          {members.length + hwMembers.length > 0 && (
            <span className="tab-badge">{members.length + hwMembers.length}</span>
          )}
        </button>
        <button
      ...
      <div className="tab-content" style={{ marginTop: 20 }}>
        ...
        {activeTab === 'members' && (
          <div className="detail-section">
            ...
            {Object.keys(membersByHw).length === 0 && <p className="text-muted">No members yet.</p>}
              ...
              return (
                ...
                >
                  ...
                  >
                    <Server size={13} />
                    {hw
                      ? `${hw.name}${hw.location ? ` — ${hw.location}` : ''}`
                      : `Hardware #${hwId}`}
                    {isHighlighted && (
                      <span
                        style={{
            ...
            {availableCUs.length === 0 && members.length === 0 && (
              <div className="info-tip" style={{ marginTop: 8 }}>
                💡 Add compute units (VMs/containers) on the <strong>Compute</strong> tab first,
                then attach them to this network.
              </div>
            )}
