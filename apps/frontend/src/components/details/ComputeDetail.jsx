      ...
      const storageResults = await Promise.all(
        svcs.data.map((svc) =>
          servicesApi
            .getStorage(svc.id)
            .then((res) =>
              (res.data || []).map((link) => ({ storage: link, serviceName: svc.name }))
            )
            .catch(() => [])
        )
      );
        ...
        {activeTab === 'overview' && (
          <div className="detail-section">
            <div className="field-group">
              ...
              <div>{compute.name}</div>
            </div>
            <div className="field-group">
              <span className="field-label">Kind</span>
              <div>{compute.kind}</div>
            </div>
            <div className="field-group">
            ...
            <div className="field-group">
              <span className="field-label">CPU / RAM</span>
              <div>
                {compute.cpu_cores} cores / {compute.memory_mb} MB
              </div>
            </div>
            <div className="field-group">
              <span className="field-label">Disk</span>
              <div>{compute.disk_gb ? `${compute.disk_gb} GB` : '—'}</div>
            </div>
            <div className="field-group">
              <span className="field-label">IP</span>
              <div>{compute.ip_address || '—'}</div>
            </div>
            {compute.proxmox_vmid != null && (
              <>
                <div className="field-group">
                  <span className="field-label">Proxmox VMID</span>
                  <div>{compute.proxmox_vmid}</div>
                </div>
                <div className="field-group">
                  <span className="field-label">PVE Type</span>
                  <div>{compute.proxmox_type === 'lxc' ? 'LXC Container' : 'QEMU VM'}</div>
                </div>
                {(() => {
                  try {
                    const ps = JSON.parse(compute.proxmox_status || '{}');
                    return ps.cpu_pct != null ? (
                      <div className="field-group">
                        <span className="field-label">Live Stats</span>
                        <div style={{ display: 'flex', gap: 12, fontSize: 13 }}>
                          ...
                          {ps.mem_used_bytes != null && ps.mem_total_bytes != null && (
                            <span style={{ color: 'var(--color-text-muted)' }}>
                              RAM {Math.round(ps.mem_used_bytes / 1024 / 1024)}/
                              {Math.round(ps.mem_total_bytes / 1024 / 1024)} MB
                            </span>
                          )}
                        </div>
                      </div>
        ...
        {activeTab === 'networking' && (
          <div className="detail-section">
            ...
            <div className="list-group">
              {networks.map((mem) => {
                const netName =
                  allNetworks.find((n) => n.id === mem.network_id)?.name ??
                  `Network #${mem.network_id}`;
                return (
                  <div
                    key={mem.network_id}
                    className="list-item"
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => handleLeaveNetwork(mem.network_id)}
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
        ...
        {activeTab === 'storage' && (
          <div className="detail-section">
            ...
            <div className="list-group">
              {storageLinks.map(({ storage, serviceName }) => {
                const capLabel = storage.capacity_gb
                  ? storage.capacity_gb >= 1024
                    ? `${(storage.capacity_gb / 1024).toFixed(1)} TB`
                    : `${storage.capacity_gb} GB`
                  : null;
                const usedPct =
                  storage.used_gb != null && storage.capacity_gb > 0
              ...
              {storageLinks.length === 0 && !compute.disk_gb && (
                <p className="text-muted">No storage allocated.</p>
              )}
            </div>
          </div>
        )}
