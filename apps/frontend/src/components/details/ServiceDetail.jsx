...
function ServiceDetail({ service, isOpen, onClose }) {
  ...
  const handleAddStorage = async () => {
    try {
      await servicesApi.addStorage(service.id, {
        storage_id: newStorageId,
        purpose: newStoragePurpose,
      });
      setNewStorageId('');
  ...
  return (
    <Drawer
      isOpen={isOpen}
      onClose={onClose}
      title={service ? `Service: ${service.name}` : 'Service Detail'}
    >
      <div className="tabs">
        <button
              ...
              {dependencies.map((dep) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => handleRemoveDep(dep.depends_on.id)}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
        ...
        {activeTab === 'resources' && (
          <div className="detail-section">
            ...
            <div className="add-row" style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              ...
              <input
                type="text"
                placeholder="Purpose (e.g. db-data)"
                value={newStoragePurpose}
                onChange={(e) => setNewStoragePurpose(e.target.value)}
                style={{ width: 140 }}
              />
              <button
                className="btn btn-sm btn-primary"
                onClick={handleAddStorage}
                disabled={!newStorageId}
              >
                Link
              </button>
            </div>
              ...
              {storageLinks.map((link) => (
                ...
                >
                  <div>
                    ...
                    <strong>{link.storage?.name}</strong>{' '}
                    <span className="text-muted">({link.purpose})</span>
                  </div>
                  <button
                    className="btn-icon danger"
                    onClick={() => handleRemoveStorage(link.storage.id)}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
            ...
            <div className="add-row" style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <select
                value={newMiscId}
                onChange={(e) => setNewMiscId(e.target.value)}
                style={{ flex: 1 }}
              >
              ...
              <input
                type="text"
                placeholder="Purpose"
                value={newMiscPurpose}
                onChange={(e) => setNewMiscPurpose(e.target.value)}
                style={{ width: 140 }}
              />
              <button
                className="btn btn-sm btn-primary"
              ...
              {miscLinks.map((link) => (
                ...
                >
                  <div>
                    ...
                    <strong>{link.misc_item?.name}</strong>{' '}
                    <span className="text-muted">({link.purpose})</span>
                  </div>
                  <button
                    className="btn-icon danger"
                    onClick={() => handleRemoveMisc(link.misc_item.id)}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
