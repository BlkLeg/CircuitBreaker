...
// `enrollment` carries the address the agent dialed. It sits on the pending
// overview because that is when the question is live — did this machine come
// in through the address I handed it? — and on the default one because an
// endpoint that stops resolving is diagnosed here long after approval. The
// terminal overview stays stripped to events: a revoked agent's enrollment
// address is history, not something to act on.
...
const PENDING_OVERVIEW = ['capabilities', 'enrollment', 'hardware', 'events'];
