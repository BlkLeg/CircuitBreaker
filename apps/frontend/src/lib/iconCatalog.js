/**
 * The shipped icon catalog, and the order its groups render in.
 *
 * Data, kept out of `IconPickerModal` — 220 entries over 900 lines, which is
 * why that component read as a 1,392-line file when the component itself is
 * under 400. `IconLibraryManager` needs the same list and was reaching into
 * the modal to get it.
 *
 * Every `path` is served from the frontend build's `/icons` mount; a slug that
 * names a file that is not there renders as a broken image, so entries and
 * assets are added together.
 */
...
/** Group render order in the picker and the library manager. `Uploaded` is
 * last because it is the only group whose contents the operator controls. */
export const GROUPS = [
  'OS',
  'Vendor',
  'Hardware',
