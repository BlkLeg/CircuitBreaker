...
// gated OnConnected to clear that marker before concluding the
...
// reExecDelayEnvOverride is a test-only escape hatch mirroring internal/link's
// rekeyIntervalEnvOverride: set to a positive integer number of milliseconds,
// onUpdate sleeps that long before re-exec'ing into the newly-swapped binary.
// The Docker E2E harness needs it to win the race against a freshly re-exec'd
// process reconnecting and self-confirming an update before the test's own
// docker-network-disconnect trigger lands — on a local bridge network,
// re-exec-to-hello.ack routinely completes in well under 100ms. No production
// deployment path sets it; unset, onUpdate re-execs immediately.
...
// ExecStart and an operator's interactive shell use. Self-update never
// touches this path directly; it only ever re-points
...
// pendingVersion to be cleared — which onConnected does exactly once, the
// moment a post-update connection reaches an accepted hello.ack, not merely a
// completed Noise handshake. If the marker is still present and still names
...
// connection to send (this process has no live link to report over — that is
// why it is rolling back), clears the marker, and re-execs via reExec.
//
// If the marker is present but never reached phasePendingConfirm
// (update.ReadMarker's swapped == false), update.Swap never ran for this
// attempt. There is nothing to roll back: currentLink was never re-pointed,
// and prevVersionDir, if the marker carries one, belongs to an earlier
// already-confirmed update — using it would silently downgrade a healthy
// running binary to a stale, unrelated version. The marker is cleared and the
// abandoned attempt logged.
//
// reExec is a parameter rather than a direct syscall.Exec so tests can observe
// a rollback decision without replacing the test binary's process image.
