  ...
  return (
    <button
      ...
      style={{
        ...
        width: 42, height: 22, borderRadius: 11, cursor: disabled ? 'not-allowed' : 'pointer',
...
export default function DiscoverySettingsPage() {
  ...
  if (!form) return <p style={{ color: 'var(--color-text-muted)', fontSize: 13, padding: '24px' }}>Loading…</p>;
  ...
  const handleResetAck = async () => {
    ...
    } catch { toast.error('Failed to reset acknowledgment'); }
  };
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <Section title="Scan Defaults">
        <SettingRow label="Default Network (CIDR)" desc="Pre-filled in the ad-hoc scan form.">
          <input className="form-input" style={{ width: 220 }} value={form.discovery_default_cidr} onChange={(e) => set('discovery_default_cidr', e.target.value)} placeholder="192.168.1.0/24" />
        </SettingRow>

        <SettingRow label="Default nmap Arguments" desc={<><span>Advanced — only change if you know nmap flags.</span></>}>
          <input className="form-input" style={{ width: 280 }} value={form.discovery_nmap_args} onChange={(e) => set('discovery_nmap_args', e.target.value)} />
        </SettingRow>

        <SettingRow label="Default SNMP Community" desc='Stored encrypted. Leave blank to use "public".'>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <input
              ...
              autoComplete="off"
            />
            <button type="button" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', padding: '4px' }} onClick={() => setShowSnmp((v) => !v)}>
              {showSnmp ? <EyeOff size={14} /> : <Eye size={14} />}
            </button>
          </div>
        </SettingRow>

        <SettingRow label="HTTP Banner Probing" desc="Probe open web ports for service identification.">
          <Toggle checked={form.discovery_http_probe} onChange={(v) => set('discovery_http_probe', v)} />
        </SettingRow>
      </Section>
      ...
      <Section title="Auto-Merge">
        {form.discovery_auto_merge && (
          <div style={{ padding: '10px 14px', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 6, marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 8, fontSize: 12, color: '#fbbf24' }}>
              <AlertTriangle size={14} style={{ flexShrink: 0, marginTop: 1 }} />
              <span>
                Auto-merge will create hardware entities automatically without review.
                Enable only on trusted, well-understood networks.
              </span>
            </div>
          </div>
        )}
        ...
        >
          <Toggle checked={form.discovery_auto_merge} onChange={(v) => set('discovery_auto_merge', v)} />
        </SettingRow>
      </Section>
      ...
      <Section title="Data Retention">
        <SettingRow label="Scan Result Retention" desc="Automatically delete scan results older than N days.">
      ...
      <Section title="Legal">
        <SettingRow
          ...
          desc={form.scan_ack_accepted
            ? 'You have acknowledged network scanning authorization.'
            : 'No acknowledgment recorded yet.'}
        >
          <button type="button" className="btn btn-secondary" style={{ fontSize: 12 }} onClick={handleResetAck}>
...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: 4 }}>
      {title && <h3 style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 16 }}>{title}</h3>}
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 24 }}>
      <div style={{ flex: 1 }}>
        ...
        {desc && <div style={{ fontSize: 11, color: 'var(--color-text-muted)', lineHeight: 1.5 }}>{desc}</div>}
      </div>
...
function Divider() {
  return <hr style={{ border: 'none', borderTop: '1px solid var(--color-border)', margin: '24px 0' }} />;
}
