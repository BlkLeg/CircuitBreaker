...
function getHostedServiceRows(hostNode, allNodes, allEdges) {
  ...
  const serviceById = new Map(
    allNodes.filter((node) => node.originalType === 'service').map((node) => [node.id, node])
  );
...
function parseLayoutData(raw) {
  ...
  return { nodes: parsed || {}, edges: {}, boundaries: [], labels: [], visualLines: [] };
}
  ...
  const autoPlaceNew = useCallback(
    async (newNodeId) => {
      try {
        ...
        toast.success('Node auto-placed safely', {
          toastId: `placed-${newNodeId}`,
          autoClose: 2000,
        });
        dirtyRef.current = true;
      } catch (e) {
          ...
          {tooltip && (
            ...
            >
              ...
              <div style={{ color: 'var(--color-text-muted)', marginBottom: 2 }}>
                Type:{' '}
                <span style={{ color: NODE_STYLES[tooltip.node.originalType]?.background }}>
                  {NODE_TYPE_LABELS[tooltip.node.originalType] || tooltip.node.originalType}
                </span>
              </div>
              ...
              {tooltip.node.data.storage_summary &&
                (() => {
                  ...
                  return (
                    <div
                      style={{ fontSize: 11, color: 'var(--color-text-muted)', marginBottom: 2 }}
                    >
              ...
              {tooltip.node.data.capacity_gb && (
                <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginBottom: 2 }}>
                  ...
                  {tooltip.node.data.capacity_gb >= 1024
                    ...
                    : `${tooltip.node.data.capacity_gb} GB`}{' '}
                  capacity
                  {tooltip.node.data.used_gb != null && tooltip.node.data.capacity_gb > 0 && (
                    <span>
                      ...
                      (
                      {Math.round(
                        (tooltip.node.data.used_gb / tooltip.node.data.capacity_gb) * 100
                      )}
                      % used)
                    </span>
                  )}
                ...
                >
                  ...
                  >
                    <div style={{ marginTop: 12 }}>
                      <div
                        style={{ marginBottom: 8, fontSize: 12, color: 'var(--color-text-muted)' }}
                      >
