...
function useSidebarWidth() {
  ...
  const onMouseDown = useCallback((e) => {
    ...
    const onMove = (mv) => {
      ...
      const next = Math.min(SIDEBAR_MAX, Math.max(SIDEBAR_MIN, startW.current + mv.clientX - startX.current));
      setWidth(next);
    };
    const onUp = () => {
...
function DocRowMenu({ doc, menuPos, onPin, onSetCategory, onDuplicate, onExport, onLink, onDelete, onClose }) {
...
function DocRightPanel({ docId, bodyMd, linksRevision = 0 }) {
  ...
  useEffect(() => {
    if (!docId) { setEntities([]); return; }
    let cancelled = false;
    setEntitiesLoading(true);
    docsApi.getDocEntities(docId)
      .then((res) => { if (!cancelled) setEntities(res.data); })
...
function DocIconPicker({ value, onChange }) {
  ...
  return (
    <div className="doc-icon-picker" ref={ref}>
      <button className="doc-icon-btn" title="Change doc icon" onClick={() => setOpen((o) => !o)} type="button">
        {value || '📄'}
      </button>
      {open && (
        <>
...
function DocsPage() {
  ...
  useEffect(() => {
    if (!linkTarget?.id) { setLinkTargetEntities([]); return; }
    docsApi.getDocEntities(linkTarget.id)
  ...
  const handleSetCategory = useCallback(async (doc) => {
    ...
    try {
      ...
      toast.error('Could not update category.'); logger.error(err);
    }
  }, [selectedDoc, fetchDocs, toast, searchQuery]);
  ...
  const handleDuplicate = useCallback(async (doc) => {
    try {
      ...
      await fetchDocs(searchQuery || undefined);
      setSelectedDoc(res.data); setEditing(false);
    } catch (err) {
      toast.error('Duplicate failed.'); logger.error(err);
    }
  }, [fetchDocs, toast, searchQuery]);

  const handleExportSingle = useCallback((doc) => {
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      ...
      logger.error(err); toast.error(err.message || 'Export failed.');
    } finally {
      setExporting(false);
    }
  }, [exporting, toast]);
  ...
  const handleImportFileChange = useCallback(async (e) => {
    ...
    try {
      ...
      await fetchDocs(searchQuery || undefined);
      if (imported.length > 0) { setSelectedDoc(imported[0]); setEditing(false); }
    } catch (err) {
      logger.error(err); toast.error(err.message || 'Import failed.');
    } finally {
      setImporting(false);
    }
  }, [fetchDocs, toast, searchQuery]);
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (selectedDoc?.id === doc.id) { setSelectedDoc(null); setEditing(false); }
  ...
  return (
    <div className="page">
      ...
      <div
        ...
        style={{ gridTemplateColumns: `${sidebar.width}px 1fr${(editing || selectedDoc) ? ' 220px' : ''}` }}
      >
        {/* Left sidebar */}
      ...
      {linkTarget && (
        <DocLinkModal
          ...
          onClose={() => { setLinkTarget(null); setLinkTargetEntities([]); }}
          onLinked={() => {
