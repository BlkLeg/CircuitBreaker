...
const PROVIDER_OPTIONS = [
  ...
  'Linode', 'Vultr', 'OVHcloud', 'Cloudflare', 'Oracle Cloud',
  'DreamHost', 'Contabo', 'Other',
];
const KIND_OPTIONS = [
  'vps', 'managed_db', 'saas', 'vpn_gateway', 'cdn', 'dns',
  'object_storage', 'serverless', 'container_registry', 'load_balancer', 'other',
];
...
function ExternalNodesPage() {
  ...
  const buildFields = useCallback((currentIcon) => [
    ...
    <div className="page">
      ...
      {!loading && items.length === 0 && settings?.show_page_hints && (
        <div className="info-tip" style={{ marginBottom: 8 }}>
          💡 <strong>Tip:</strong> Add external/cloud nodes (VPS, managed databases, SaaS dependencies) here. You can link them to your local networks and services.
        </div>
      )}
      ...
      <Drawer
        isOpen={!!detailTarget}
        onClose={() => { setDetailTarget(null); setNodeNetworks([]); setNodeServices([]); }}
          ...
          <div className="detail-content">
            <div className="detail-section">
              ...
              {detailTarget.icon_slug && (
                <p><IconImg slug={detailTarget.icon_slug} size={28} /> <span style={{ color: 'var(--color-text-muted)', fontSize: 12 }}>{detailTarget.icon_slug}</span></p>
              )}
              <p><strong>Provider:</strong> {detailTarget.provider || '—'}</p>
              <p><strong>Kind:</strong> {detailTarget.kind || '—'}</p>
              <p><strong>Region:</strong> {detailTarget.region || '—'}</p>
              <p><strong>IP:</strong> {detailTarget.ip_address || '—'}</p>
              <p><strong>Environment:</strong> {detailTarget.environment || '—'}</p>
              <p><strong>Tags:</strong> {(detailTarget.tags || []).join(', ') || '—'}</p>
              {detailTarget.notes && <p><strong>Notes:</strong> {detailTarget.notes}</p>}
            </div>

            <div className="detail-section">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h4>Connected Networks ({nodeNetworks.length})</h4>
                <button className="btn btn-sm" onClick={() => setShowLinkNetworkForm(true)}>+ Link Network</button>
              </div>
                ...
                <ul className="link-list">
                  {nodeNetworks.map((link) => (
                    <li key={link.id}>
                      ...
                      <button className="btn btn-sm btn-danger" onClick={() => handleUnlinkNetwork(link.id)} title="Unlink">×</button>
                    </li>
                  ))}
                </ul>
              )}
      ...
      <FormModal
        ...
        fields={[
          {
            name: 'network_id', label: 'Network', type: 'select', required: true,
            options: networks.map((n) => ({ value: n.id, label: `${n.name}${n.cidr ? ` (${n.cidr})` : ''}` })),
          },
          {
            name: 'link_type', label: 'Link Type', type: 'select',
