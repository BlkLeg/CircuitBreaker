...
const CATEGORY_OPTIONS = ['infrastructure', 'web', 'monitoring', 'database', 'remote_access', 'misc'];
...
export default function ServiceChecklistModal({ hardwareId, hardwareName, ports, onClose }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success(`${toCreate.length} service${toCreate.length !== 1 ? 's' : ''} added to ${hardwareName}`);
      onClose();
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 600 }}>Add Services for {hardwareName}?</h3>
        ...
        <p style={{ margin: '0 0 16px', fontSize: 12, color: 'var(--color-text-muted)' }}>
          We found the following open ports. Select which ones to add as services linked to this device.
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {rows.map((row, idx) => (
            <div key={idx} style={{
              ...
              background: row.checked ? 'rgba(var(--color-primary-rgb,0,212,255),0.06)' : 'transparent',
              border: '1px solid var(--color-border)',
            }}>
              <input
                type="checkbox"
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button type="button" className="btn btn-secondary" onClick={onClose} disabled={saving}>Skip</button>
