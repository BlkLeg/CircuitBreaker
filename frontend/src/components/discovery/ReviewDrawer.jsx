...
function DiscoveredBadge() {
  return (
    <span style={{
      ...
      border: '1px solid rgba(59,130,246,0.3)', marginLeft: 6,
    }}>◆ Discovered</span>
  );
}
...
export default function ReviewDrawer({ result, onClose, onAccepted, onRejected }) {
  ...
  const [name, setName]              = useState(result.hostname || result.snmp_sys_name || result.ip_address || '');
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--color-text-muted)' }}>
              Source: {result.scan_job_id ? `scan job #${result.scan_job_id}` : 'network scan'} · {result.ip_address}
            </p>
          </div>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                {['hardware', 'compute'].map((t) => (
                  <label key={t} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 13 }}>
                    <input type="radio" name="entityType" value={t} checked={entityType === t} onChange={() => setEntityType(t)} />
          ...
          {isNew && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <Field label="Name" badge={!edited.name}>
                <input className="form-input" value={name} onChange={(e) => { setName(e.target.value); markEdited('name'); }} />
              </Field>
              <Field label="IP Address">
                <input className="form-input" value={result.ip_address} readOnly style={{ opacity: 0.6, cursor: 'not-allowed' }} />
              </Field>
              <Field label="MAC Address" badge={!edited.mac && !!mac}>
                <input className="form-input" value={mac} onChange={(e) => { setMac(e.target.value); markEdited('mac'); }} />
              </Field>
              <Field label="Vendor">
              ...
              <Field label="OS / Notes" badge={!edited.os && !!osNotes}>
                <input className="form-input" value={osNotes} onChange={(e) => { setOsNotes(e.target.value); markEdited('os'); }} />
              </Field>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button type="button" className="btn btn-danger" onClick={handleReject} disabled={submitting}>Reject</button>
