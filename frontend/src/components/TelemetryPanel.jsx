...
export default function TelemetryPanel({ hardwareId }) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const res = await telemetryApi.pollNow(hardwareId);
      setResult((prev) => ({ ...prev, ...res }));
      setMsg(res.status === 'healthy' ? 'Connection OK — device reachable.' : `Connected — status: ${res.status}`);
    } catch (err) {
  ...
  return (
    <details
      ...
      style={{ marginTop: 24, border: '1px solid var(--color-border)', borderRadius: 8, padding: '10px 14px' }}
    >
      <summary style={{
        cursor: 'pointer', fontWeight: 600, fontSize: 13,
        color: 'var(--color-text)', listStyle: 'none', display: 'flex',
      ...
      {open && (
        <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '8px 12px', alignItems: 'center' }}>
            ...
            <select value={profile} onChange={(e) => setProfile(e.target.value)} style={{ fontSize: 12 }}>
            ...
            <input
              ...
              placeholder="192.168.1.10" style={{ fontSize: 12, fontFamily: 'monospace' }}
            />

            {/* Auth fields — iLO only */}
            {needsAuth && (<>
              <label style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>Username</label>
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} style={{ fontSize: 12 }} />
              <label style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} style={{ fontSize: 12 }} />
            </>)}

            {/* SNMP community — everything else */}
            {needsSNMP && (<>
              <label style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>SNMP Community</label>
              <input type="text" value={snmpCommunity} onChange={(e) => setSnmpCommunity(e.target.value)} style={{ fontSize: 12 }} />
            </>)}

            {/* Poll interval */}
            <label style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>Poll Interval (s)</label>
            <input
              type="number" min={10} value={pollInterval}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
            <button className="btn btn-primary btn-sm" onClick={handleSaveConfig} disabled={saving || !host}>
              {saving ? 'Saving…' : 'Save Config'}
            </button>
            <button className="btn btn-sm" onClick={handleTestConnection} disabled={saving || !host}>
              Test Connection
            </button>
          ...
          {/* Inline feedback */}
          {msg && (
            <p style={{ margin: 0, fontSize: 12, color: msg.startsWith('Error') || msg.startsWith('Poll failed') || msg.startsWith('Connection failed') ? '#ef4444' : '#22c55e' }}>
              {msg}
            </p>
          )}
          ...
          {result?.last_polled && (
            <div style={{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 4 }}>
              ...
              {result.data?.cpu_temp && <span style={{ marginLeft: 12 }}>CPU: {result.data.cpu_temp}°C</span>}
              {result.data?.system_power_w && <span style={{ marginLeft: 12 }}>Power: {result.data.system_power_w}W</span>}
            </div>
          )}
        </div>
      )}
