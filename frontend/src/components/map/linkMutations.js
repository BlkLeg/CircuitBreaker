...
const UPDATABLE_EDGE_PREFIXES = ['e-ext-net-', 'e-svc-ext-', 'e-dep-', 'e-ss-', 'e-sm-', 'e-cn-', 'e-hn-', 'e-hh-'];
...
async function createLinkDirectional(sourceNode, targetNode) {
  ...
  const srcType = sourceNode.originalType;
  const targetType = targetNode.originalType;

  if (srcType === 'service') {
    if (targetType === 'hardware') {
      await servicesApi.update(srcId, { hardware_id: tgtId });
      return edgeMeta({ relation: 'hosts', edgePrefix: 'e-hw-svc-', sourceNodeId: `hw-${tgtId}`, targetNodeId: `svc-${srcId}`, updatable: false });
    }
    if (targetType === 'compute') {
      await servicesApi.update(srcId, { compute_id: tgtId });
      return edgeMeta({ relation: 'runs', edgePrefix: 'e-cu-svc-', sourceNodeId: `cu-${tgtId}`, targetNodeId: `svc-${srcId}`, updatable: false });
    }
    if (targetType === 'storage') {
      await servicesApi.addStorage(srcId, { storage_id: tgtId });
      return edgeMeta({ relation: 'uses', edgePrefix: 'e-ss-', sourceNodeId: `svc-${srcId}`, targetNodeId: `st-${tgtId}`, updatable: true });
    }
    if (targetType === 'misc') {
      await servicesApi.addMisc(srcId, { misc_id: tgtId });
      return edgeMeta({ relation: 'integrates_with', edgePrefix: 'e-sm-', sourceNodeId: `svc-${srcId}`, targetNodeId: `misc-${tgtId}`, updatable: true });
    }
    if (targetType === 'network') {
      if (sourceNode._computeId) {
        await networksApi.addMember(tgtId, { compute_id: sourceNode._computeId });
        return edgeMeta({ relation: 'on_network', edgePrefix: 'e-svc-net-', sourceNodeId: `svc-${srcId}`, targetNodeId: `net-${tgtId}`, updatable: false });
      }
      if (sourceNode._hwId) {
    ...
    if (targetType === 'external') {
      ...
      return edgeMeta({ relation: 'depends_on', edgePrefix: 'e-svc-ext-', sourceNodeId: `svc-${srcId}`, targetNodeId: `ext-${tgtId}`, updatable: true });
    }
  }
  ...
  if (srcType === 'compute') {
    if (targetType === 'hardware') {
      ...
      return edgeMeta({ relation: 'hosts', edgePrefix: 'e-hw-cu-', sourceNodeId: `hw-${tgtId}`, targetNodeId: `cu-${srcId}`, updatable: false });
    }
    if (targetType === 'service') {
      await servicesApi.update(tgtId, { compute_id: srcId });
      return edgeMeta({ relation: 'runs', edgePrefix: 'e-cu-svc-', sourceNodeId: `cu-${srcId}`, targetNodeId: `svc-${tgtId}`, updatable: false });
    }
    if (targetType === 'network') {
      await networksApi.addMember(tgtId, { compute_id: srcId });
      return edgeMeta({ relation: 'connects_to', edgePrefix: 'e-cn-', sourceNodeId: `cu-${srcId}`, targetNodeId: `net-${tgtId}`, updatable: true });
    }
  }
  ...
  if (srcType === 'hardware') {
    if (targetType === 'hardware') {
      ...
      return edgeMeta({ relation: 'connects_to', edgePrefix: 'e-hh-', sourceNodeId: `hw-${srcId}`, targetNodeId: `hw-${tgtId}`, updatable: true });
    }
    if (targetType === 'compute') {
      await computeUnitsApi.update(tgtId, { hardware_id: srcId });
      return edgeMeta({ relation: 'hosts', edgePrefix: 'e-hw-cu-', sourceNodeId: `hw-${srcId}`, targetNodeId: `cu-${tgtId}`, updatable: false });
    }
    if (targetType === 'storage') {
      await storageApi.update(tgtId, { hardware_id: srcId });
      return edgeMeta({ relation: 'has_storage', edgePrefix: 'e-hw-st-', sourceNodeId: `hw-${srcId}`, targetNodeId: `st-${tgtId}`, updatable: false });
    }
    if (targetType === 'cluster') {
      await clustersApi.addMember(tgtId, { hardware_id: srcId });
      return edgeMeta({ relation: 'cluster_member', edgePrefix: 'e-cluster-', sourceNodeId: `cluster-${tgtId}`, targetNodeId: `hw-${srcId}`, updatable: false });
    }
    if (targetType === 'network') {
      await networksApi.addHardwareMember(tgtId, { hardware_id: srcId });
      return edgeMeta({ relation: 'on_network', edgePrefix: 'e-hn-', sourceNodeId: `hw-${srcId}`, targetNodeId: `net-${tgtId}`, updatable: true });
    }
  }
  ...
  if (srcType === 'network') {
    if (targetType === 'hardware') {
      ...
      return edgeMeta({ relation: 'on_network', edgePrefix: 'e-hn-', sourceNodeId: `hw-${tgtId}`, targetNodeId: `net-${srcId}`, updatable: true });
    }
    if (targetType === 'compute') {
      await networksApi.addMember(srcId, { compute_id: tgtId });
      return edgeMeta({ relation: 'connects_to', edgePrefix: 'e-cn-', sourceNodeId: `cu-${tgtId}`, targetNodeId: `net-${srcId}`, updatable: true });
    }
    if (targetType === 'service') {
      ...
      return edgeMeta({ relation: 'on_network', edgePrefix: 'e-svc-net-', sourceNodeId: `svc-${tgtId}`, targetNodeId: `net-${srcId}`, updatable: false });
    }
    if (targetType === 'external') {
      await externalNodesApi.addNetwork(tgtId, { network_id: srcId });
      return edgeMeta({ relation: 'connects_to', edgePrefix: 'e-ext-net-', sourceNodeId: `ext-${tgtId}`, targetNodeId: `net-${srcId}`, updatable: true });
    }
  }
  ...
  if (srcType === 'storage') {
    if (targetType === 'service') {
      ...
      return edgeMeta({ relation: 'uses', edgePrefix: 'e-ss-', sourceNodeId: `svc-${tgtId}`, targetNodeId: `st-${srcId}`, updatable: true });
    }
    if (targetType === 'hardware') {
      await storageApi.update(srcId, { hardware_id: tgtId });
      return edgeMeta({ relation: 'has_storage', edgePrefix: 'e-hw-st-', sourceNodeId: `hw-${tgtId}`, targetNodeId: `st-${srcId}`, updatable: false });
    }
  }
  ...
  if (srcType === 'misc') {
    if (targetType === 'service') {
      ...
      return edgeMeta({ relation: 'integrates_with', edgePrefix: 'e-sm-', sourceNodeId: `svc-${tgtId}`, targetNodeId: `misc-${srcId}`, updatable: true });
    }
  }
  ...
  if (srcType === 'external') {
    if (targetType === 'network') {
      ...
      return edgeMeta({ relation: 'connects_to', edgePrefix: 'e-ext-net-', sourceNodeId: `ext-${srcId}`, targetNodeId: `net-${tgtId}`, updatable: true });
    }
  }
...
export async function unlinkByEdge(edge) {
  ...
  if ((rel === 'on_network' || rel === 'connects_to') && src.prefix === 'cu' && tgt.prefix === 'net') {
