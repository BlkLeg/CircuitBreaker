...
function VisualLineContextMenu({
  position,
  lineType,
  onChangeType,
  onDelete,
  ...
  return (
    ...
    >
      <div className="tw-px-4 tw-py-2 tw-border-b tw-border-cb-border tw-bg-cb-secondary">
        <div className="tw-font-mono tw-font-bold tw-text-cb-text tw-text-sm tw-truncate" style={{ textTransform: 'capitalize' }}>
          {lineType} Line
        </div>
      ...
      <div className="tw-py-1">
        <div className="tw-px-4 tw-py-2">
          <div className="tw-text-xs tw-text-cb-text tw-mb-2 tw-uppercase tw-tracking-wider">Line Type</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
            {CONNECTION_TYPE_OPTIONS.map((type) => {
              ...
              return (
                <button
                  key={type}
                  onClick={() => { onChangeType(type); onClose(); }}
                  style={{
                    padding: '5px 4px',
                    borderRadius: 5,
                    border: type === lineType
                      ? `2px solid ${cs?.stroke || '#555'}`
                      : '1px solid var(--color-border)',
                    background: type === lineType ? 'var(--color-glow)' : 'var(--color-surface-secondary)',
                    color: cs?.stroke || '#ccc',
                    fontSize: 9,
                    fontWeight: 700,
                    cursor: 'pointer',
        ...
        <button onClick={() => { onDelete(); onClose(); }} className={rowDanger}>
