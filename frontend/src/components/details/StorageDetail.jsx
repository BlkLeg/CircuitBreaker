  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title={`Storage: ${storage.name}`}>
      ...
      <div className="tab-content" style={{ marginTop: 20 }}>
            ...
            {storage.used_gb != null && (
              <div className="field-group">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  {storage.capacity_gb > 0 && (() => {
                    const pct = Math.min(100, Math.round((storage.used_gb / storage.capacity_gb) * 100));
                    const barColor = pct > 85 ? 'var(--color-danger)' : pct > 65 ? '#f59e0b' : 'var(--color-online)';
                    return (
        ...
        {activeTab === 'docs' && (
          <DocsPanel entityType="storage" entityId={storage.id} />
        )}
      </div>
    </Drawer>
  );
