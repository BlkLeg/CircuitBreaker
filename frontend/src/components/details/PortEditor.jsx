...
const ConnectionSelector = ({ value, onChange, hardwareId, excludedHardwareIds = [] }) => {
  ...
  const selectedOption = useMemo(() => {
    ...
    if (value.connected_hardware_id) {
      const found = options.find(o => o.type === 'hardware' && o.data.id === value.connected_hardware_id);
      return found;
    } else if (value.connected_compute_id) {
      const found = options.find(o => o.type === 'compute' && o.data.id === value.connected_compute_id);
      return found;
    }
    return null;
  }, [value, options]);
  ...
  return (
    <Select
      ...
      styles={{
        control: (base) => ({ ...base, minHeight: '32px', borderColor: 'var(--color-border)', boxShadow: 'none', '&:hover': { borderColor: 'var(--color-primary)' } }),
        ...
        menu: (base) => ({ ...base, background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '6px', zIndex: 100 }),
...
function PortEditor({ hardware, onSave, onCancel }) {
  ...
  const excludedHardwareIds = useMemo(() => [
    hardware.id,
    ...ports.filter((p, idx) => idx !== editPortIdx && p.connected_hardware_id).map(p => p.connected_hardware_id),
  ...
  const handlePortFormCancel = () => {
    if (editPortIdx !== null && ports[editPortIdx]?.label === `Port ${ports[editPortIdx]?.port_id}` && !ports[editPortIdx]?.connected_hardware_id && !ports[editPortIdx]?.connected_compute_id) {
  ...
  const portFormFields = useMemo(() => [
    ...
    { name: 'type', label: 'Type', type: 'select', options: PORT_TYPES.map(pt => ({ value: pt.value, label: pt.label })) },
    { name: 'speed_mbps', label: 'Speed (Mbps)', type: 'number' },
    {
        name: 'connection', label: 'Connects To', type: 'custom',
        render: (formValues, updateValues) => (
            <ConnectionSelector 
                value={{ connected_hardware_id: formValues.connected_hardware_id, connected_compute_id: formValues.connected_compute_id }}
  ...
  return (
    <div className="port-editor-panel">
          ...
          ports.map((p, idx) => (
            ...
            >
              <div className="port-header">
                ...
                <button className="port-remove-btn" onClick={(e) => { e.stopPropagation(); handleRemovePort(idx); }}>
                  <X size={14} />
                </button>
              </div>
      ...
      <FormModal
        open={showPortForm}
        title={editPortIdx !== null ? `Edit Port ${currentPortToEdit.label || currentPortToEdit.port_id}` : 'New Port'}
