...
def pick(releases: list[dict], *, channel: str = "stable") -> dict | None:
...
def test_picks_newest_even_though_an_older_release_is_flagged_stable():
    ...
    """
    assert pick(LIVE, channel="candidate")["tag_name"] == "v1.0.0-rc.4"
...
def test_never_falls_back_to_the_newest_stable_during_a_candidate_window():
    ...
    assert pick(LIVE, channel="candidate")["tag_name"] != "v0.3.4"
...
def test_skips_drafts():
    ...
    assert pick(releases, channel="candidate")["tag_name"] == "v1.0.0-rc.4"
...
def test_candidate_channel_takes_the_newest_release_of_either_kind():
    """candidate keeps the pre-channel rule: newest non-draft of either kind."""
    releases = [
    ...
    assert pick(releases, channel="candidate")["tag_name"] == "v1.0.1-rc.1"


def test_stable_channel_skips_prereleases():
    """stable — the default — never silently installs a candidate."""
    releases = [
        release("v1.0.1-rc.1", prerelease=True),
        release("v1.0.0"),
        release("v1.0.0-rc.4", prerelease=True),
...
def test_the_candidate_warning_does_not_claim_no_stable_release_exists():
    """The warning fires on the candidate channel when the winner is a prerelease."""
