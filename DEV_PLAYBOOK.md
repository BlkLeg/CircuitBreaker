...
### Install-script vs compose stack

The repo has two ways to run Circuit Breaker in Docker:

| Deployment | Container names | Image source | Managed by |
|---|---|---|---|
| **install.sh** | `circuit-breaker`, `cb-caddy-proxy` | `ghcr.io/blkleg/circuitbreaker:latest` (prebuilt) | `docker run` (standalone) |
| **docker compose** | `cb-backend`, `cb-frontend`, `cb-caddy` | Built from local source | `docker compose` |

These two deployments **conflict** if both are running — they share port 8080/80/443 and the Caddy proxy name. The `make` targets handle this automatically, but if you run into issues:

```bash
make dev-stop-install   # stop the install-script container (data preserved)
docker start circuit-breaker  # restart it later when done with compose
```

### `make docker-build`
**Use when:** You want to build Docker images from local source without starting anything.

Builds both the unified `circuit-breaker:beta` image (root `Dockerfile`) and the compose service images (`cb-backend`, `cb-frontend`). This ensures `make compose-up` and `make preflight` always test against your latest code.

```bash
make docker-build
# Produces: circuit-breaker:beta, circuitbreaker-backend:latest, circuitbreaker-frontend:latest
```

### `make dev-stop-install`
**Use when:** Switching from the install-script deployment to the compose dev stack.

Stops the `circuit-breaker` container deployed by `install.sh` (if running) to free up ports and avoid name collisions. The data volume `circuit-breaker-data` is preserved — restart later with `docker start circuit-breaker`.

Called automatically by `compose-up` and `compose-fresh`.

### `make compose-up`
**Use when:** Testing the full containerised stack locally.

Automatically stops any install-script container first, then rebuilds both images from source and starts the compose stack. The database persists across restarts. Served via Caddy reverse proxy.

```
App → https://circuitbreaker.local (if TLS configured)
    → http://localhost (port 80 via Caddy)
```
...
Automatically stops any install-script container first, wipes all volumes (`down -v`), then rebuilds and starts the stack. The app will present the OOBE wizard on next browser load.

```bash
make compose-fresh
# Open the app → OOBE wizard appears
```

### Troubleshooting: scans find 0 hosts in Docker

The default nmap arguments include `-O` (OS detection), which requires `CAP_NET_RAW`. Inside Docker without that capability, nmap refuses to run. The backend automatically detects this and falls back to unprivileged arguments (`-sT` connect scan instead of SYN scan, `-O` stripped). If scans still find nothing:

1. Verify the container can reach the target subnet (`docker exec cb-backend ping -c1 <target-ip>`).
2. With Docker Desktop, the container runs inside a VM — it cannot reach your LAN unless you use host networking.
3. To enable ARP scanning (native Linux Docker only), uncomment `cap_add` and `network_mode: "host"` in `docker/docker-compose.yml`.
...
Runs `test` → `frontend-build` → `docker-build` (builds both the unified `circuit-breaker:beta` image and the compose service images). All three must pass before publishing.
...
## Docker Images workflow (compose-prod)

`.github/workflows/docker.yml` builds the backend and frontend images for the compose-prod stack and pushes them to GHCR.
...
**How to test without pushing to main:** Go to **Actions → Docker Images → Run workflow**, choose your branch, and run. The workflow builds from the selected branch and pushes images with the `-latest` suffix (or `-<tag>` when run from a tag). Use this to verify image builds before merging.
...
| `make docker-build` | Build unified + compose images from source | No |
| `make dev-stop-install` | Stop install-script container (data kept) | No |
| `make compose-up` | Stop install container + rebuild + start stack | No |
| `make compose-down` | Stop Docker stack, keep data | No |
| `make compose-clean` | Stop + wipe all volumes | **Yes** |
| `make compose-fresh` | Stop install container + wipe + rebuild (OOBE) | **Yes** |
...
This runs `test` → `frontend-build` → `docker-build` (unified image + compose images). All three must be green.

**Also verify the Docker stack end-to-end:**

```bash
make compose-fresh     # stops install container + wipe + rebuild — triggers OOBE wizard
# Open the app, complete OOBE, smoke-test your changes
```

> `compose-up` and `compose-fresh` automatically stop the install-script `circuit-breaker` container if it's running. You don't need to do this manually.
