...
def prometheus_metrics(
    ...
    for kind, count in (
        db.query(ComputeUnit.kind, func.count(ComputeUnit.id))
        .group_by(ComputeUnit.kind)
        .all()
    ):
    ...
    for kind, count in (
        db.query(Storage.kind, func.count(Storage.id))
        .group_by(Storage.kind)
        .all()
    ):
    ...
    capacity_total.set(
        db.query(func.coalesce(func.sum(Storage.capacity_gb), 0)).scalar() or 0
    )
    ...
    used_total.set(
        db.query(func.coalesce(func.sum(Storage.used_gb), 0)).scalar() or 0
    )
    ...
    for level, category, count in (
        ...
        log_entries.labels(
            level=level or "unknown", category=category or "unknown"
        ).set(count)
    ...
    services = db.query(
        Service.name, Service.slug, Service.environment, Service.status
    ).all()
