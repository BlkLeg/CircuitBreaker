...
async def upload_user_icon(
    ...
    if existing and existing[1]:
        return {"filename": existing[1], "url": f"/user-icons/{existing[1]}", "icon_id": existing[0]}
    ...
    icon_id = db.execute(text("SELECT id FROM user_icons WHERE hash = :hash"), {"hash": file_hash}).scalar()
    db.commit()
...
async def upload_favicon(
    ...
    db.execute(text("UPDATE app_settings SET favicon_path = :path WHERE id = 1"), {"path": "/branding/favicon.ico"})
    db.commit()
