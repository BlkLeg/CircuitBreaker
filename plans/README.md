...
| [Step 4 — packaging toolchain](./2026-09-20-step4-packaging-toolchain.md) | 2026-09-20 | **Superseded** — by [`docs/superpowers/specs/2026-09-22-installer-and-release-design.md`](../docs/superpowers/specs/2026-09-22-installer-and-release-design.md) and its plan. Task 1 (benchmark) and Task 2 (decision) landed (`docs/evidence/2026-09-20-packaging-benchmark.md`); its Task 3a/3b split is replaced by the PBS cutover, whose measurement is appended to that evidence file under "Decision (2026-09-22)". |
...
| Plan | Date | Status |
|---|---|---|
| [Technical debt cleanup](./2026-09-07-tech-debt-cleanup.md) | 2026-09-07 | **Active** — phased prune, split, test/doc alignment, and contributor-readiness work. No product features. Phases 1–2 landed in earlier sessions (the plan's own "Phase 2 exit criteria — all met, 2026-09-07", plus the Map rework sections). Phases 3–4 mostly landed in the `tech-cleanup` branch's commits (`e23488fc..14c9df79`): the test-suite pruning and splits, and `ce93c8ca`/`27af1f68`/`f5133abe`/`7d15a8b4` (architecture page, CHANGELOG, feature-request form, PR template, CONTRIBUTING). **Four §3/§4 items remain open, with no disposition recorded anywhere:** `good first issue` labelling (a GitHub account action, out of scope for an in-tree change); the §3.1 Playwright visual-baseline re-snap (still 18 PNGs — regenerated 2026-09-07 for the unrelated font-leak fix, not for the agent-console/nav UI moves §3.1 actually asks about); `docs/overview.md`'s Discovery labelling (still "(Beta)" at :60/:62/:69, §3.2's "confirm against product intent" never answered); and moving `specs/2026-09-06-site-scoping-design.md` etc. (§3.2's "confirm with maintainers" never recorded). Not Complete while those stand. |
| [Architectural feasibility](./2026-09-07-architectural-feasibility.md) | 2026-09-07 | **Reference** — post-cleanup assessment of layer lint, plugins, repositories, TypeScript, OpenAPI codegen, and OpenTelemetry. Not a work item. Assumes cleanup Phases 1–4 have landed; as of `14c9df79` most has, but the four items named in the row above are still open and undisposed. |
...
| Plan | Date | Status |
|---|---|---|
| [Unattended agent enrollment (slice B)](./2026-09-06-agent-unattended-enrollment-slice-b.md) | 2026-09-06 | **Complete** — all twelve tasks landed: `f242eba8` (1, the table), `05d5284a` (2–3, mint/list/consume/revoke), `e9206f50` (4, the admin routes), `74ed5e4c` (5, enrolling from a token), `b28a9983` (6, the consumption race), `af183b5d` (7–8, the installer carries the token and the command passes it), `b9ac57a6` (9, the agent presents and erases it), `7d7937bc` (10, the wizard's unattended branch), `8a5a9dc1` (11, tokens in settings), `47ae2b84` + `bc8e2bf2` + `1aaa1f5c` (12, the `cbe_` scanner rule, the docs, and the end-to-end proof), with the endpoint inventory reconciled in `2e5882cb`. The plan's own deferral stands: the token arrives through `CB_ENROLL_TOKEN` only, never stdin, because the shipped command already pipes the script into `sh`. **Unattended enrollment has passed the E2E suite but has not been run against a real second machine** — the 2026-09-06 deployment to `ubuntu26` enrolled attended. Slice B of [`docs/design/2026-09-05-agent-reachability-design.md`](../docs/design/2026-09-05-agent-reachability-design.md), and depends on slice A: a token is scoped to an endpoint. |
