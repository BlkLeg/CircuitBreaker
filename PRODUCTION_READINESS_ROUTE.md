...
| F3 unsigned agent binaries | **Fixed in working tree, shipping in warn mode** | Detached Ed25519 signature over each binary, verified against a key embedded at build time via `-ldflags -X` — not configurable at runtime, not delivered by the server, not read from disk, since a key the server can influence reproduces the finding. Signing runs in the release job from `secrets.AGENT_SIGNING_PRIVATE_KEY`; a T0 ratchet asserts both halves reach the build step, because a signer that is defined but never invoked is the `age`-had-no-caller defect again. **Warn until 0.6.0**: agents running today carry no embedded key, so defaulting to refusal would break every in-flight fleet. A build with no key stays in warn mode even under the enforce flag — otherwise every self-built agent strands the day the default flips | Phase 4.2 done |
| F4 TLS pin rotation unsolved | **Fixed** | `tls.pin.rotate` frame both sides; one `link.ResolveTrust` resolver for all four dial sites, T0-ratcheted; admin rotate/status/pending routes; certificate activation gated on fleet convergence with an audited `force`. Two things widened the finding beyond what this row assumed: the rotated unit had to be a **policy `(mode, pin)`**, not a pin — a pin-only advertisement cannot say "stop pinning", so a self_signed↔public cutover strands the fleet in *both* directions — and the fourth dial site is the **update download**, which is what makes a stranding unrepairable remotely rather than merely inconvenient | Phase 4.1 done |
...
| F6 route-layer DB access | **Gated, not fixed** | Re-measured 2026-09-01 by AST, not grep: **581** session-op calls across 43 files. The 354 came from a pattern this document never records and which does not reproduce. Frozen by `tests/build/test_phase3_boundary_ratchets.py`; the count can now only fall | Phase 3.1 done |
| F7 core→services inversion | **Gated, not fixed** | Re-measured: **1 top-level** (`core/destructive_actions.py:10`) and **21 deferred** across 9 modules — not the 12 this document assumed. The split is load-bearing: a top-level import inverts the dependency at import time, an in-function one is the idiom for breaking that cycle. The gate bans new top-level imports outright and freezes the deferred count | Phase 3.1 done |
| F8 models.py monolith | Still verified | 2,846 lines / 85 classes, unchanged | Deferred |
| F9 main.py lifespan | **Partly addressed** | Was 2,542 lines at Phase 3 start (not 2,519). Of 29 scheduled jobs, 19 already referenced imported functions and **10 were closures defined inline in the lifespan** — the closures are the finding, since one cannot be imported or tested. The 5 Proxmox closures are extracted to `app/jobs/proxmox.py` with tests; `main.py` is 2,466. **5 inline closures remain** and are follow-up | Phase 3.4 partial |
| F10 WS auth duplication | **Fixed for the duplicated four; `ws_monitors` deliberately untouched** | This row's framing was backwards. `ws_topology`, `ws_telemetry`, `ws_discovery` and `ws_agents` each carried a byte-identical handshake — that is the duplication, now one `api/ws_session.py`. `ws_monitors`'s `jwt.decode` **is** correctly pinned (HS256 + audience) and it accepts *more* identity kinds (service-account JWTs gated on liveness, API tokens) plus a `read:*` scope check. Folding it in would widen the four to accept API tokens or narrow it — a privilege change, not a refactor | Phase 3.2 done |
...
| F13 silent exception swallows | **Gated, not fixed** | Re-measured by AST: **119** handlers whose body is exactly `pass`/`...`, across 45 files (a docstring above the `pass` does not stop it being silent). Frozen at 119 | Phase 3.1 done |
| F14 no dead-letter on JetStream consumers | **Fixed** | Both consumers declare `max_deliver=5`; failure handling is per-message, not per-batch (`monitor_poll_worker` naked whole batches, so one poison message blocked every good message beside it). Exhausted messages park in `failed_messages` with their payload and are listed/requeued/discarded through admin routes. The park commits **before** `term()`, and a failed park naks rather than terminating | Phase 3.3 done |
...
| F17 agent_events outside hash chain | **Fixed** | `record_event` routes a frozen set of ten authorization events through `log_service.write_log`, which already owns the hashing, the advisory lock and the never-raises contract. The set is deliberately narrow and T0-pinned: the chain takes a **global** `pg_advisory_xact_lock` per write, so chaining the agent-initiated high-volume events (`connected`, `protocol_violation`, …) would trade a write-amplification problem for an instance-wide contention one | Phase 4.3 done |
...
| F24 unthrottled protocol_violation writes | **Fixed** | Routed through `recordable_violation` like the capability path beside it. The throttle windows per **(agent, kind)**, not per agent: suppressing repeats is the goal, but a first attempt keyed on agent alone collapsed a distinct `unsupported_version` into a `duplicate_sequence` count and broke an existing end-to-end assertion | Phase 3.1 done |
...
| Slice | T | L | C | E | D | Notes |
|---|---|---|---|---|---|---|
| 3.1 Ratchet tests (T0, `tests/build/test_phase3_boundary_ratchets.py`) — **done 2026-09-01**: direct-DB-in-`api/` frozen at **581** (not 354), new top-level `core→services` imports banned with 1 allowlisted and **21** deferred frozen (not a 12-module allowlist), `except: pass` frozen at **119** (not 107). The token-issuer ratchet was already shipped in Phase 1 (`test_phase1_policy_ratchets.py`) and was listed here in error | 3 | 4 | 5 | 2 | 4 | Counts only go down. Each gate was proven to fail on a deliberately injected violation before being trusted |
...
| Slice | T | L | C | E | D | Notes |
|---|---|---|---|---|---|---|
| 4.1 TLS pin-successor rotation (F4) — **done 2026-09-02**: additive `tls.pin.rotate` frame over the authenticated Noise channel, mirroring the server-key state machine; convergence-gated certificate activation; runbook at `docs/tls-trust-rotation.md`; E2E green. **Four defects were found building it, and all four are worth recording because none of them failed anything** — the mechanism looked correct at every layer and did nothing: (1) the successor pin was derived through `_tls_mode_and_pin`, which prefers the *live* certificate, so the rotation advertised the pin the fleet already had; (2) convergence was keyed on an agent *matching* the successor, which is only observable after the cutover the gate guards, so every rotation would have had to be forced; (3) promotion cleared the successor without recording it, so an agent survived exactly one connection past the cutover and then stranded — F4 itself, one step later; (4) the peer certificate was read from gorilla's dial response, whose `TLS` field is always nil, so the promotion path never ran at all. Only the E2E could find (4) | 5 | 4 | 5 | 3 | 4 | Fleet-bricking landmine; first because 4.2's rollout depends on agents surviving cert churn |
| 4.2 Agent binary signing (F3) — **done 2026-09-02**: detached Ed25519 signature verified before the swap and before the rollback marker, so a refused update leaves nothing behind; signing key lives in the release pipeline, never in the web app runtime; warn mode now, enforce at 0.6.0. `test_agent_signature_encoding.py` pins the contract between the Python signer and the Go verifier — they are in different languages and neither suite alone would catch an encoding mismatch, since the Go tests sign their own fixtures | 5 | 2 | 5 | 3 | 3 | Retires the largest blast radius |
| 4.3 Chain agent authorization events (F17) into the hash-chained `Log` — **done 2026-09-02**: ten events chained as an *additional* write beside the timeline row the agent UI reads, the set frozen by a T0 ratchet proven to fail on a deliberately added high-volume event | 4 | 2 | 3 | 2 | 2 | Cheap once touched |
...
| Dependency-direction ratchets (**active**): new top-level `core→services` imports banned (1 allowlisted) and deferred ones ≤**21**; direct-DB-in-`api/` ≤**581**; `except: pass` ≤**119**. Counts are AST-derived and re-derivable, replacing the unreproducible grep figures this table previously carried (354/107) | Boundary drift | seconds | Low; counts only ever tighten | T0 | backend |
...
| Release checklist (hard stops): all T0–T3 green with evidence hashes; every row named in `specs/1.0.0/release-control/release-blockers.csv` reads `status=passed` with `invalidation_state=current`; restore rehearsal done; changelog + rollback instructions published; no open B1–B6 condition. **Corrected 2026-09-03:** this row previously said "ledger has no `not_evidenced` blocker rows". The ledger has no blocker or severity column, and `not_evidenced` is an `invalidation_state`, not a `status` — so the condition named nothing the CSV could answer, and 115 of 145 rows are `not_evidenced` today. The blocker set is now declared in its own file and the T0 validator asserts those ids exist, so a rename cannot silently orphan the checklist. Enforcement at release time is still the release owner's, not automated | Process bypass | minutes | n/a | release | release owner |
...
- [x] Instrumentation from Phase 2.1 shipped (request IDs, loop-lag, nav ring buffer) — production-safe mode only. Delivered beyond the line as written: **per-chunk fetch telemetry** (§4.2), which had never been built and without which §4.4's first YES branch was unobservable, and **DB pool occupancy gauges plus a `pool_timeout` counter**, which §5 lists as measured but the backend did not export.
- [x] The three Verified navigation defects fixed (H5 429s, H6 whole-app unmount, H7 `/map` wedge) and the §4 harness run recorded. **Wedge rate is nonzero and must appear in the release notes: 12/30 (40%) at 6× CPU throttle**, every instance a navigation away from `/map`, all on H1's branch. This is the before-baseline, not a fixed defect — H1's remediation is R1, which Phase 2 does not ship. `make nav-wedge` reproduces it.
- [ ] Baseline program producing evidence: the nightly Tier A/B/C job is committed, non-blocking, and archives results JSON, but **it cannot fire where it currently lives**: GitHub runs `schedule` only from the default branch, and `baseline.yml` is on `dev`, so no push to `dev` will start it — `workflow_dispatch` is the only way to produce a result before it merges to `main`, and Phase 4.5's four-week clock does not start until it does — no baseline evidence exists yet, and none of the earlier reports count (their server-side fields were all null; see §3 Phase 2). Per ruling **R-A** the fortnight of accumulation is not a Phase 2 obligation, only a green archiving job is. Per ruling **R-B** the **agent and telemetry-rate axes of §5 are not measured** — synthetic Noise-protocol agents are deferred — and every result file names them as deferred rather than omitting them. Phase 4.4 therefore cannot make agent-ingest decisions on data.
...
- [x] F4 pin-successor rotation shipped + E2E (cert regeneration must not strand agents). The rotated unit is a policy `(mode, pin)`, not a pin, and certificate activation is gated on fleet convergence rather than on the operator remembering the order. Two defects found while building the gate and fixed rather than shipped — see the F4 row in §2.
- [x] F3 signed agent updates in **warn** mode, enforce announced for **0.6.0**; the signing key lives in the release pipeline's secret store and is never read by the application runtime. A T0 ratchet asserts the release job actually invokes the signer, because a signer that is defined and never called ships a feature that has never run.
- [x] F17 agent authorization events in the hash-chained audit log — ten events, frozen by a T0 ratchet.
- [x] F24 violation-write throttling (delivered in Phase 3.1).
- [x] Documented recovery runbook: compromised-agent revocation (`docs/agent.md` "Revoke and uninstall"), server-key rotation (`docs/agent-key-rotation.md`), TLS trust rotation (`docs/tls-trust-rotation.md`), and the Redis-outage behavior F16 asks be stated plainly (`docs/agent.md` recovery scenario 7 — the agent plane fails closed and does not degrade, which is deliberate and now says so).

**Ledger, recorded not promoted.** `AGT-07` moves to `in_progress` with the **re-key** and **failed update** clauses named as exercised by the new E2E scenarios; its other eight clauses are untouched. `AGT-16`'s **audited** half is met and its **explicit confirmation** half is a UI clause this work does not reach. Neither row passes on this work alone, and recording them at their real state is the honest outcome.
