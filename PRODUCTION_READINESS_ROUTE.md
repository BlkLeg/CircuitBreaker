...
| Slice | T | L | C | E | D | Notes |
|---|---|---|---|---|---|---|
| 4.1 TLS pin-successor rotation (F4) — **done 2026-09-02**: additive `tls.pin.rotate` frame over the authenticated Noise channel, mirroring the server-key state machine; convergence-gated certificate activation; runbook at `docs/tls-trust-rotation.md`. Two defects found while building the gate and fixed rather than shipped: convergence was keyed on an agent *matching* the successor, which is only observable after the cutover the gate guards, so every rotation would have had to be forced; and promotion cleared the successor without recording it, so an agent survived exactly one connection past the cutover and then stranded — F4 itself, reintroduced one step later | 5 | 4 | 5 | 3 | 4 | Fleet-bricking landmine; first because 4.2's rollout depends on agents surviving cert churn |
